...
include_once('../../../data/db_pdo.php'); // 데이터 베이스 접속 불러오기
  ...
  // 1. 리스트에 출력하기 위한 sql문
  $list_sql = video_list($first, $list_size);
...
?>
  ...
  <h4>공지사항 게시판입니다.</h4>
  <div id="write_btn">
      <a href="./video_w_u.php"><button>글쓰기</button></a>
  </div>
    <table>
      <thead>
          <tr>
...
<?php
  while($board=$list_stt->fetch())  {
              ...
              $title=$board["video_title"]; 
              if(strlen($title)>30){ 
                //title이 30을 넘어서면 ...표시
                $title=str_replace($board["video_title"],mb_substr($board["video_title"],0,30,"utf-8")."...",$board["video_title"]);
              } 
 ?>
    ...
    <tbody>
      <tr>
        ...
        <td width="500"><a href="./video_w_u.php?no=<?php echo $board['no'];?>"><?php echo $title;?></a></td>
        <td width="100"><?php echo $board['video_write_date']?></td>
